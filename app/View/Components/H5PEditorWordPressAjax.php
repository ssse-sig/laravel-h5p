...
class H5PEditorWordPressAjax extends Component implements H5PEditorAjaxInterface  {
...
//            AND hl3.minor_version = hl4.minor_version");
        ...
        // essendo la lista delle librerie gestita dalla scuola ci limitiamo a caricare tutte le librerie
        // ignorando le versioni, avremo molto probabilmente solo una versione per ogni libreria
        // nel nostro database.

//        $libraries = \DB::table('wp_h5p_libraries as hl')
//            ->select('hl.id',
//                'hl.name AS machine_name',
//                'hl.title',
//                'hl.major_version',
//                'hl.minor_version',
//                'hl.patch_version',
...
//        return $libraries2;
            ...
            ->where('hl.runnable','1')->join('wp_h5p_libraries_hub_cache as lhc','hl.name', 'lhc.machine_name')
//           ->max('major_version as major_version)
//           ->groupBy('hl.name')
            ->get();
        $i =0;
...
//        dd($libraries2);
    ...
    public function getContentTypeCache($machineName = NULL) {
        ...
        return $this->getLatestLibraryVersions();
    }
    ...
    public function getAuthorsRecentlyUsedLibraries() {
        global $wpdb;
//        $recently_used = array();
        $results = \DB::table('wp_h5p_events')
            ->where('user_id', 1)
            ->where('type', 'content')
        ...
        foreach ($results as $res){
            $recently_used[$i]=get_object_vars($res)['library_name'];
            $i++;
        }
//        dd($recently_used);
