...
class H5PContentValidator extends Component
{
    public $h5pF;
    public $h5pC;
...
//        dd($metadata);
    ...
    public function validateLibrary(&$value, $semantics) {
        ...
        if (!in_array($value->library, $libraryNames)) {
            ...
            $nameArray = explode(' ', $value->library);
            $name = $nameArray[0];
            foreach ($libraryNames as $semanticsLibrary) {
                $semanticsnameArray = explode(' ', $semanticsLibrary);
                $semanticsname = $semanticsnameArray[0];
                if ($name === $semanticsname) {
                    // Using the wrong version of the library in the content
                    $message = $this->h5pF->t('The version of the H5P library %name used in this content is not valid. Content contains %contentLibrary, but it should be %semanticsLibrary.', array(
                        '%name' => $name,
        ...
        if (!isset($this->libraries[$value->library])) {
            ...
            $library = $this->h5pC->loadLibrary($libSpec['name'], $libSpec['major_version'], $libSpec['minor_version']);
            $library['semantics'] = $this->h5pC->loadLibrarySemantics($libSpec['name'], $libSpec['major_version'], $libSpec['minor_version']);
