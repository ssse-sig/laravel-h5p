...
//include("App\View\Components\h5p-php-library\h5p.classes.php");
 ...
 * @author Joubel <contact@joubel.com>
 */
class H5P_Plugin extends Component{
...
//        add_option('h5p_has_request_user_consent', FALSE);
    ...
    public function get_h5p_path() {
        $upload_dir =  'app\View\Components';
        return $upload_dir;
    }
...
//        return $absolute ? $url[$id]['abs'] : $url[$id]['rel'];
        ...
        /* TO DO: rendere dinamici questi url*/
        $url['abs'] = url('/') . '/h5p/uploads';
        $url['rel']= 'h5p/uploads';
        return $url['abs'];
    }
...
//        }
    ...
    public function get_h5p_instance($type) {
        ...
        if (empty(self::$interface[$id])) {
            ...
            self::$core[$id] = new H5PCore(self::$interface[$id], $this->get_h5p_path(), $this->get_h5p_url(), $language, false);
...
//            'exportUrl' => url('/').'\/h5p\/uploads\/exports\/test-nuovo-4.h5p',
            'exportUrl' => url('/').'\/h5p\/uploads\/exports\/test-nuovo-4.h5p',
            'embedCode' => '<iframe src=\"http://h5pdawp2.test/2/4?id=4" width=\":w\" height=\":h\" frameborder=\"0\" allowfullscreen=\"allowfullscreen\"><\/iframe>',
            'resizeCode' => '<script src=\"http://h5pdawp2.test\h5p\/h5p-php-library\/js\/h5p-resizer.js\" charset=\"UTF-8\"></script>',
            'url' =>'http://h5pdawp2.test/2/4?id=4',
            'title' => $content['title'],
...
//        if (get_option('h5p_save_content_state', FALSE)) {
...
//            ));
            $results =DB::table('wp_h5p_contents_user_data')->where('user_id', 1)->where('content_id',$content['id'])->get();
...
//        }
        ...
        foreach ($dependencies as $dependency) {
            $dep = $dependency;
            if(!is_array($dep)) {
                $dep = get_object_vars($dependency);
                // Wordpress ha deciso di utilizzare nel suo codice un metodo diverso rispetto da H5P
                // per nominare certi valori creando un po' di scompiglio.
                $dep['machineName'] = $dep['name'];
                $dep['majorVersion'] =$dep['major_version'];
                $dep['minorVersion'] =$dep['minor_version'];
            }
//            dd($dep);
            $libraries[$dep['machineName']] = array(
//            $libraries[get_object_vars($dependency['machineName'])] = array(
...
//        return $settings;
        ...
        $settings = array(
            ...
            'ajax' => array( /*admin_url('admin-ajax.php?token=' . wp_create_nonce('h5p_result') . '&action=h5p_setFinished')*/
                'setFinished' => url('/').'/writeresults?_token='.csrf_token().'&action=h5p_setFinished',
                'contentUserData' => ''),
            'saveFreq' => FALSE,
            'siteUrl' => url('/'),//csrf_field()
            'l10n' => array(
                'H5P' => $core->getLocalization(),
            ),
            'hubIsEnabled' => true,
            ...
            'crossoriginCacheBuster' => defined('H5P_CROSSORIGIN_CACHE_BUSTER') ? H5P_CROSSORIGIN_CACHE_BUSTER : null,
            'pluginCacheBuster' => '?v=' . self::VERSION,
            'libraryUrl' => 'http://h5pdawp2.test/h5p/h5p-php-library/js'
        );
        if (1) {
            $settings['user'] = array(
                'name' => 'admin',
