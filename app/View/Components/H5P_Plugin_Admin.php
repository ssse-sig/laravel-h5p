 ...
 * @author Joubel <contact@joubel.com>
 */
class H5P_Plugin_Admin extends  Component{
...
//                        $scripts[] = 'http://h5pdawp2.test/h5p/h5p-php-library/' . $script . $cache_buster;
                        $scripts[] =  url('/').'/h5p/h5p-php-library/' . $script . $cache_buster;
                    }

                    // Get core styles
                    $styles = array();
                    foreach (H5PCore::$styles as $style) {
//                        $styles[] = 'http://h5pdawp2.test/h5p/h5p-php-library/' . $style . $cache_buster;
                        $styles[] =  url('/').'/h5p/h5p-php-library/' . $style . $cache_buster;
                    }
//                    dd($scripts);
//                    dd($integration);
...
//                    do_action_ref_array('h5p_additional_embed_head_tags', array(&$additional_embed_head_tags));
                    ...
                    include_once("h5p/h5p-php-library/embed.php");
//                    dd($core);
                    // Log embed view
        ...
        print '<body style="margin:0"><div style="background: #fafafa url( "h5p/h5p-php-library/images/h5p.svg") no-repeat center;background-size: 50% 50%;width: 100%;height: 100%;"></div><div style="width:100%;position:absolute;top:75%;text-align:center;color:#434343;font-family: Consolas,monaco,monospace">' . __('Content unavailable.', $this->plugin_slug) . '</div></body>';
        exit;
    }
...
//      $site_key = get_option('h5p_site_key', get_option('h5p_h5p_site_uuid', FALSE));
    ...
    public static function  add_script($handle, $path) {
        // to do: add this scripts somehow if this breaks something
//        self::asset_handle($handle);
//        var_dump($path);
        $script = url('/').    '/h5p/' . $path .'?'. H5P_Plugin::VERSION;
        return $script;
...
//        $user_id = get_current_user_id();
        $user_id = 1;
        $result_id = \DB::table('wp_h5p_results')->select('id')->where('user_id',$user_id)->where('content_id',$content_id)->get();
//        dd(get_object_vars($result_id[0])['id']);
        $newResult =false;
        try {
            $result_id = get_object_vars($result_id[0])['id'];
        }catch(\Exception $ex){
                $newResult = true;
                $result_id = null;
            }
...
//        }
        ...
        $content = \DB::table('wp_h5p_contents as c')
            ->where('c.id', $content_id)
            ->join('wp_h5p_libraries as l', 'l.id','=','c.library_id')
...
//        dd($content);
        $content = $content[0];//$content->get('items');
