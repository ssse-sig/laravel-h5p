...
class H5PWordPress extends Component implements H5PFrameworkInterface {
...
//        ddd(vsprintf(__($message, $this->plugin_slug), $replacements));
...
//        $upload_dir = wp_upload_dir();
        return url('/'). '/h5p/uploads/libraries/' . $libraryFolderName . '/' . $fileName;
    }
...
//        dd($sql_args);
        $id = DB::table('wp_h5p_libraries')
            ->select('id')
            ->where('name',$sql_args[0])
            ->where('major_version',$sql_args[1])
            ->where('minor_version',$sql_args[2])
            ->get();
...
//            'disable' => $content['disable']
        ...
        // TO DO: Rimuovere questo id di default, necessario solo nella fase di testing.
        if($id == null){
            $id=1;
        }
...
//            $wpdb->insert($table, $data, $format);
        ...
        new H5P_Event('content', $event_type,
            ...
            $content['library']['major_version'] . '.' . $content['library']['minor_version']);
...
//        $wpdb->delete($wpdb->prefix . 'h5p_contents', array('id' => $id), array('%d'));
        DB::table('wp_h5p_contents')->select('*')->where('id',$id)->delete();
...
//        $wpdb->delete($wpdb->prefix . 'h5p_results', array('content_id' => $id), array('%d'));
        DB::table('wp_h5p_results')->select('*')->where('content_id',$id)->delete();

        // Remove contents user/usage data
//        $wpdb->delete($wpdb->prefix . 'h5p_contents_user_data', array('content_id' => $id), array('%d'));
        DB::table('wp_h5p_contents_user_data')->select('*')->where('content_id',$id)->delete();
    }
...
//        $wpdb->delete($wpdb->prefix . 'h5p_contents_libraries', array('content_id' => $contentId), array('%d'));
       ...
       DB::table('wp_h5p_contents_libraries')->select('*')->where('content_id',$contentId)->delete();
    }
...
//        dd($contentId);
        ...
        foreach ($librariesInUse as $dependency) {
            ...
            DB::table('wp_h5p_contents_libraries')->insert([
...
//            );
    ...
    public function loadLibrary($name, $majorVersion, $minorVersion) {
        ...
        $library = DB::table('wp_h5p_libraries')->select("id as libraryId",
            "name as machineName",
            "title"
        ...
        $library = get_object_vars($library[0]);
//        dd($library);
...
//            ARRAY_A
//        );
//        dd( $library['libraryId']);

        $dependencies = DB::table("wp_h5p_libraries_libraries as hll")
            ...
            ->join("wp_h5p_libraries as hl","hll.required_library_id","=","hl.id")
...
//            $library['libraryId'])
//        );
//        $dependencies = get_object_vars($dependencies);
//        $listsize = $dependencies->count();
        $dependencies = $dependencies->toArray();
//        dd($dependencies);
//        for ($i = 0; $i < $listsize;$i++ ){
//            $dependencies[$i] = get_object_vars( $dependencies[$i]);
//        }
...
//        dd($dependencies);
    ...
    public function loadLibrarySemantics($name, $majorVersion, $minorVersion) {
        ...
        if ($this->isInDevMode()) {
            ...
            $semantics = DB::table('wp_h5p_libraries')
                ->select("semantics")
                ->where("name", $name)
                ->where('major_version',$majorVersion)
                ->where('minor_version', $minorVersion)
...
//            );
        ...
        $semantics = get_object_vars($semantics[0])['semantics'];
//        dd($semantics);
...
//        dd($id);
        global $wpdb;
//        dd();
        $content = DB::table('wp_h5p_contents as hc')
            ->select('hc.id'
              , 'hc.title'
            ...
            ->OrderBy('hc.id','ASC')
            ->join('wp_h5p_libraries as hl', 'hc.library_id', 'hl.id')
            ->where('hc.id','=',$id);
        $content2 = NULL;
        $content = $content->get()->toArray();
        if(!empty($content)){
            $content2 = get_object_vars($content[0]);
...
//        dd($content2);
    ...
    public function loadContentDependencies($id, $type = NULL) {
        global $wpdb;
        $query = DB::table('wp_h5p_contents_libraries as hcl')
            ->join('wp_h5p_libraries as hl', 'hcl.library_id', 'hl.id')
...
//        }
        $queryArgs[] = $type;
        if ($type !== NULL) {
            $query->where('hcl.dependency_type',$queryArgs );
        }

//        $query .= " ORDER BY hcl.weight";
        $query->orderBy('hcl.weight');
...
//            array('%d'));
        ...
        DB::table('wp_h5p_contents')->where('id',$id)->update($processedFields);
    }
    ...
    public function getNumNotFiltered() {
        ...
        $result = DB::table('wp_h5p_contents')
            ->where('filtered','=','')->count('id');
...
//        );
    ...
    public function fetchExternalData($url, $data = NULL, $blocking = TRUE, $stream = NULL) {
        ...
        // sto provando a rimuovere l'intero metodo, sembra essere inutile per noi.
//        @set_time_limit(0);
//        $options = array(
...
//        }
    ...
    public function resetContentUserData($contentId) {
        ...
        DB::table('wp_h5p_contents_user_data')
...
//        return !$wpdb->get_var($wpdb->prepare("SELECT slug FROM {$wpdb->prefix}h5p_contents WHERE slug = '%s'", $slug));
        $queryslug = null;
        try {
            $queryslug =  DB::table('wp_h5p_contents')->select('slug')->where('slug',$slug)->get()[0]->slug;
        }catch (\Exception $ex){
            return true;
        }
    ...
    public function saveCachedAssets($key, $libraries)
    {
        ...
        try {
            ...
            foreach ($libraries as $library) {
                $library = get_object_vars($library);
                // TODO: Avoid errors if they already exists...
                DB::table('wp_h5p_libraries_cachedassets')->insert(
                    array(
...
//        if (current_user_can('edit_others_h5p_contents')) {
    ...
    public function hasPermission($permission, $contentUserId = NULL) {
        ...
        // per ora ignoro completamente i diritti.
        Return true;
    ...
    public function loadAddons() {
        ...
        $addon = DB::table("wp_h5p_libraries as l1")->select(
            "l1.id as libraryId",
            ...
            ->leftJoin('wp_h5p_libraries as l2',"l1.name","=", "l2.name")
...
//        );
    ...
    public function libraryHasUpgrade($library) {
        ...
        $result= DB::table('wp_h5p_libraries')
            ->select('id')
            ->where('name',$library['name'])
            ->where('major_version','>',$library['major_version'])
            ->orWhere('major_version','=', $library['major_version'])
            ->where('name',$library['name'])
            ->where('minor_version','>',$library['minor_version'])
            ->get();
