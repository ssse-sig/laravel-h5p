...
class H5PCore extends Component
{
...
//            dd($content);
            $content['library'] = array(
                ...
                'major_version' => $content['libraryMajorVersion'],
                'minor_version' => $content['libraryMinorVersion'],
...
//            }
            ...
            $content['title'] = \DB::table('wp_h5p_contents')->select('title')->where('id',$content['id'])->get()[0]->title;
//            dd($content['title']);
            $content['slug'] = $this->generateContentSlug($content);
...
//            $dependency =  get_object_vars($dependency);
            ...
            try {
                $dependency['preloadedJs'] = $dependency['preloaded_js'];
                $dependency['preloadedCss'] = $dependency['preloaded_css'];
                $dependency['majorVersion'] = $dependency['major_version'];
                $dependency['minorVersion'] = $dependency['minor_version'];
                $dependency['patchVersion'] = $dependency['patch_version'];
            ...
            try {
                $dependency['preloaded_js'] = $dependency['preloadedJs'];
                $dependency['preloaded_css'] = $dependency['preloadedCss'];
                $dependency['major_version'] = $dependency['majorVersion'];
                $dependency['minor_version'] = $dependency['minorVersion'];
                $dependency['patch_version'] = $dependency['patchVersion'];
...
//                dd($dependency);
            ...
            $dependency['version'] = "?ver={$dependency['major_version']}.{$dependency['minor_version']}.{$dependency['patch_version']}";
    ...
    private static function getDependenciesHash(&$dependencies) {
        ...
        foreach ($dependencies as $dep) {
            ...
            $toHash[] = "{$dep['name']}-{$dep['major_version']}.{$dep['minor_version']}.{$dep['patch_version']}";
        }
    ...
    public function loadLibrarySemantics($name, $major_version, $minor_version) {
        ...
        if (isset($this->h5pD)) {
            ...
            $semantics = $this->h5pD->getSemantics($name, $major_version, $minor_version);
        }

        if ($semantics === NULL) {
            // Try to load from DB.
            $semantics = $this->h5pF->loadLibrarySemantics($name, $major_version, $minor_version);
        }

        if ($semantics !== NULL) {
            $semantics = json_decode($semantics);
            $this->h5pF->alterLibrarySemantics($semantics, $name, $major_version, $minor_version);
        }
    ...
    public function loadLibrary($name, $major_version, $minor_version) {
        ...
        if (isset($this->h5pD)) {
            ...
            $library = $this->h5pD->getLibrary($name, $major_version, $minor_version);
//            dd($library);
            if ($library !== NULL) {
                $library['semantics'] = $this->h5pD->getSemantics($name, $major_version, $minor_version);
            }
        }

        if ($library === NULL) {
            // Try to load from DB.
            $library = $this->h5pF->loadLibrary($name, $major_version, $minor_version);
        }
//        dd($library);
        return $library;
    }
...
//            dd($library);
            ...
            foreach ($library[$property] as $dependency) {
                $dependencyKey = $type . '-' . $dependency['name'];
                if (isset($dependencies[$dependencyKey]) === TRUE) {
                    continue; // Skip, already have this.
                }
//                dd($dependencies);
                $dependencyLibrary = $this->loadLibrary($dependency['name'], $dependency['major_version'], $dependency['minor_version']);
     ...
     *
     * Same version means that the major_version and minor_version is the same
     *
     * @param array $library
    ...
    public function isSameVersion($library, $dependency) {
        if ($library['name'] != $dependency['name']) {
            return FALSE;
        }
        if ($library['major_version'] != $dependency['major_version']) {
            return FALSE;
        }
        if ($library['minor_version'] != $dependency['minor_version']) {
            return FALSE;
        }
        return TRUE;
    }
    ...
    /**
     * Writes library data as string on the form {name} {major_version}.{minor_version}
     *
     * @param array $library
     *  With keys name, major_version and minor_version
     * @param boolean $folderName
     *  Use hyphen instead of space in returned string.
     * @return string
     *  On the form {name} {major_version}.{minor_version}
     */
    public static function libraryToString($library, $folderName = FALSE) {
//        dd($library);

        // questi controlli devo farlo a causa di un'incompatibilità tra i nomi usati da h5p e quelli usati da WordPress.
        // TO DO: Scegliere uno solo dei due tipi di nomenclatura e rimpiazzare la precedenta.
        try {
            if ($library['name'] !== Null) {
                $library['machineName'] = $library['name'];
            }
            if ($library['major_version'] !== Null) {
                $library['majorVersion'] = $library['major_version'];
            }
            if ($library['minor_version'] !== Null) {
...
//        dd($library);
        return (isset($library['machineName']) ? $library['machineName'] : $library['machineName']) . ($folderName ? '-' : ' ') . $library['majorVersion'] . '.' . $library['minorVersion'];
    }

    /**
     * Parses library data from a string on the form {name} {major_version}.{minor_version}
     *
...
//      );
        ...
        if (isset($result->libraries)) {
            foreach ($result->libraries as $library) {
                if (isset($library->tutorialUrl) && isset($library->name)) {
                    $this->h5pF->setLibraryTutorialUrl($library->namee, $library->tutorialUrl);
                }
            }
        }
    ...
    public function getLibraryId($library, $libString = NULL) {
        ...
        if (!isset($libraryIdMap[$libString])) {
            $libraryIdMap[$libString] = $this->h5pF->getLibraryId($library['name'], $library['major_version'], $library['minor_version']);
        }
